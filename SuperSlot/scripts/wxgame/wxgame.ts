    ...
    async onFile(file: plugins.File) {
        if (file.extname == '.js') {
            ...
            if (filename == 'libs/modules/egret/egret.js' || filename == 'libs/modules/egret/egret.min.js') {
                ...
                if (filename.indexOf("pureMvc") >= 0) {
                    content += ";window.puremvc = puremvc;"
                }
                if (filename.indexOf("uniLib") >= 0) {
                    content += "\n;window.uniLib = uniLib;";
                    content += ";window.Pmd = Pmd;"
                    // content += ";window.pako = pako;"
                    content += ";window.wx = wx;"
                }
                if (filename.indexOf("chessCommonLib") >= 0) {
                    content = content.replace(/window.chessCommonlib={};/gi, "var chessCommonLib = {};");
                    content = content.replace(/window.chessCommonLib/gi, "chessCommonLib");
                    content += ";window.chessCommonLib = chessCommonLib;";
                    content += ";window.Cmd = Cmd;"
                }
                content = "var egret = window.egret;" + content;
                if (filename == 'main.js') {
                    content += ";window.SuperSlotMain = SuperSlotMain;"
                    content += ";window.superslot = superslot;"
                    content += ";window.SZLobby = SZLobby;"
                    content = content.replace(/var Cmd;/gi, "var Cmd=window.Cmd;");
                }
                file.contents = new Buffer(content);
            }
        }
